...
int main() {
    ...
    try {
        common::generate_packet_sequence("var_stream_id.vrt", &p, N_PACKETS, [](uint64_t i, vrt_packet* p) {
            p->header.packet_type  = VRT_PT_IF_DATA_WITH_STREAM_ID;
            p->header.has.class_id = false;
            p->fields.stream_id    = i % 4;
        });
        common::generate_packet_sequence("var_def_stream_id.vrt", &p, N_PACKETS, [](uint64_t i, vrt_packet* p) {
            p->header.has.class_id = false;
            if (i % 4 == 0) {
        ...
        common::generate_packet_sequence("var_oui.vrt", &p, N_PACKETS, [](uint64_t i, vrt_packet* p) {
            p->header.has.class_id = true;
            p->header.packet_type  = VRT_PT_IF_DATA_WITHOUT_STREAM_ID;
            p->fields.class_id.oui = i % 4;
        });
        common::generate_packet_sequence("var_icc.vrt", &p, N_PACKETS, [](uint64_t i, vrt_packet* p) {
            p->header.has.class_id                    = true;
            p->header.packet_type                     = VRT_PT_IF_DATA_WITHOUT_STREAM_ID;
            p->fields.class_id.information_class_code = i % 4;
        });
        common::generate_packet_sequence("var_pcc.vrt", &p, N_PACKETS, [](uint64_t i, vrt_packet* p) {
            p->header.has.class_id               = true;
            p->header.packet_type                = VRT_PT_IF_DATA_WITHOUT_STREAM_ID;
            p->fields.class_id.packet_class_code = i % 4;
        });
        common::generate_packet_sequence("var_class_id.vrt", &p, N_PACKETS, [](uint64_t i, vrt_packet* p) {
            p->header.has.class_id                    = true;
            p->header.packet_type                     = VRT_PT_IF_DATA_WITHOUT_STREAM_ID;
        ...
        common::generate_packet_sequence("var_def_class_id.vrt", &p, N_PACKETS, [](uint64_t i, vrt_packet* p) {
        ...
        common::generate_packet_sequence("var_class_stream_id.vrt", &p, N_PACKETS, [](uint64_t i, vrt_packet* p) {
            p->header.has.class_id                    = true;
            p->header.packet_type                     = VRT_PT_IF_DATA_WITH_STREAM_ID;
            p->fields.class_id.oui                    = i % 2;
            p->fields.class_id.information_class_code = i % 2;
        ...
        common::generate_packet_sequence("var_def_class_stream_id.vrt", &p, N_PACKETS, [](uint64_t i, vrt_packet* p) {
            if (i % 3 == 0) {
                p->header.packet_type = VRT_PT_IF_DATA_WITHOUT_STREAM_ID;
                p->fields.stream_id   = i % 4;
            } else {
        ...
        common::generate_packet_sequence("var_hex.vrt", &p, N_PACKETS, [](uint64_t i, vrt_packet* p) {
            p->header.has.class_id = true;
            p->header.packet_type  = VRT_PT_IF_DATA_WITH_STREAM_ID;
            if (i % 2 == 0) {
